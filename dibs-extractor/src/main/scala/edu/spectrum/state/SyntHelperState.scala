...
object SyntHelperState {
    ...
    if (files.length > 0) {
      files.headOption foreach {
        f =>
          ...
          p ! ("SYNTHelperState - handleFileList", path + " to " + dest.getPath + " has been copied successfuly...")
      }
      true
    } else
      false
  ...
  def handleWorkspace(path: String) = {
    ...
    if (wDir.isDirectory) {
      if (!handleFileList(wDir, new FilenameFilter(".abn"), new File(Constants.ABN_PATH))) {
        ...
        p ! ("SYNTHelperState - handleWorkspace", Constants.DEF_SOLAR_ABN + " to " + Constants.ABN_PATH + " has been copied successfuly... Default settings!")
      }
      if (!handleFileList(wDir, new FilenameFilter("synt.inp"), new File(Constants.SYNTH_PATH))) {
        ...
        p ! ("SYNTHelperState - handleWorkspace", Constants.DEF_SYNTH_INP + " to " + Constants.SYNTH_PATH + " has been copied successfuly... Default settings!")
      }
      if (!handleFileList(wDir, new FilenameFilter("input.atl"), new File(Constants.MODEL_PATH))) p ! ("SYNTHelperState - handleWorkspace", "Kurucz's format model file is not found... Some error? Check workspace directory or file names there...")
      if (!handleFileList(wDir, new FilenameFilter(".obs"), new File(Constants.OBS_DATA))) p ! ("SYNTHelperState - handleWorkspace", "Observational data file is not found... Some error? Check workspace directory or file names there...")
      if (!handleFileList(wDir, new FilenameFilter("convolve.conf"), new File(Constants.CONV_PATH))) {
        ...
        p ! ("SYNTHelperState - handleWorkspace", Constants.DEF_CONV_CONF + " to " + Constants.CONV_PATH + " has been copied successfuly... Default settings!")
      }
    }
  }
  ...
  def saveSettings() = {
    ...
    p ! ("Saving Settings", "Settings have been saved successfuly...")
  }
  ...
  def copySettingsFilesToWorkspace() = {
    ...
    FileUtils.copyFile(src, new File(workspacePath + File.separator + src.getName))
    p ! ("Export Settings Files to Workspace", "Files have been copied successfuly...")
  }
