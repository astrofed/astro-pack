...
class MainFrame extends JFrame {
  ...
  def setValues() = {
    MainFrame.statusBar.setMainMessage(workspacePath)
  ...
  private def controls() = {
    ...
    layout.setHorizontalGroup(layout.createParallelGroup(Alignment.LEADING).addComponent(tabbedPane, GroupLayout.DEFAULT_SIZE, 640, MAX_VALUE).addComponent(MainFrame.statusBar, GroupLayout.DEFAULT_SIZE, 640, MAX_VALUE))
    layout.setVerticalGroup(layout.createParallelGroup(Alignment.TRAILING).addGroup(layout.createSequentialGroup.addContainerGap().addComponent(tabbedPane, GroupLayout.DEFAULT_SIZE, 357, MAX_VALUE).addComponent(MainFrame.statusBar, GroupLayout.PREFERRED_SIZE, 25, GroupLayout.PREFERRED_SIZE)))
  ...
  private def menu() = {
      ...
      def actionPerformed(e: ActionEvent) {
        new Thread(new Runnable {
          def run() = {
            ...
            val path: String = MainFrame.openDialogFileChooser(SyntHelperState.workspacePath, dirOnly = true, "Choose Workspace", "Choose", "")
    ...
    mntmDIB.addActionListener {
      new ActionListener {
        def actionPerformed(arg0: ActionEvent) {
          new Thread(new Runnable {
            def run() = {
              val path: String = MainFrame.openDialogFileChooser(SyntHelperState.workspacePath, dirOnly = false, "Choose spectrum contains DIB", "Choose", "")
    ...
    mntmLogWindow.addActionListener {
      new ActionListener {
        def actionPerformed(arg0: ActionEvent)  = p.openWindow()
      }
    }
  ...
  private def render() = {
    ...
    menu()
    controls()
    p! "Welcome to  SYNTHelper! Logging started..."
  }
